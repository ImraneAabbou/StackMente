...
import type { ReplyReport } from "@/types/report";
...
type GrouppedReplyReport = ReplyReport & {
    reports_count: number
}

export default function AdminReportsReplies() {
    const { setAction } = useContext(ConfirmDeleteCtx)
    const { reports: { items: reportItems, next_page_url } } = usePage().props
    const [reports, setReports] = useState(reportItems as GrouppedReplyReport[])
    ...
    return <AdminLayout>
        <div className="flex sm:flex-row flex-col gap-8 justify-between sm:items-center">
            <div className="flex flex-col">
                <h1 className="text-2xl font-display">{t("reports.reported_replies_title")}</h1>
                <p className="text-secondary">{t("reports.reported_replies_desc")}</p>
            </div>
            <SearchInput />
        </div>
        <section className="mt-8">
            {
                !reports.length
                    ...
                    : <table className="min-w-full divide-y divide-secondary/25">
                        <thead>
                            <tr>
                                <th scope="col" className="py-3.5 text-left text-sm font-semibold">{t("content.reply_by")}</th>
                                    ...
                                    <td className="relative whitespace-nowrap py-4 text-right text-sm font-medium sm:pr-0">
                                        <Menu as="div">
                                            ...
                                            >
                                                ...
                                                <MenuItem>
                                                    <button
                                                        onClick={() => setAction(route("replies.destroy", { reply: r.reportable_id }))}
                                                        className="group flex w-full items-center gap-2 rounded-lg py-1.5 px-3 hover:bg-background-light dark:hover:bg-background-dark text-error-light dark:text-error-dark">
                                                        <Trash size={16} />
                                                        {t("content.delete")}
                                                    </button>
            ...
            }
            {
                ...
                <InfiniteScrollLoader
                    url={next_page_url}
                    onSuccess={
                        p => { setReports([...reports, ...(p.reports.items as GrouppedReplyReport[])]) }
                    }
