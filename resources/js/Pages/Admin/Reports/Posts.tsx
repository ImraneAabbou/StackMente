...
type GrouppedPostReport = PostReport & {
...
export default function AdminReportsPosts() {
    ...
    const [reports, setReports] = useState(reportItems as GrouppedPostReport[])
    ...
    return <AdminLayout>
        <div className="flex sm:flex-row flex-col gap-8 justify-between sm:items-center">
            <div className="flex flex-col">
                <h1 className="text-2xl font-display">{t("reports.reported_posts_title")}</h1>
                <p className="text-secondary">{t("reports.reported_posts_desc")}</p>
            </div>
            <SearchInput />
        </div>
        <section className="mt-8">
            {
                !reports.length
                    ...
                    : <table className="min-w-full divide-y divide-secondary/25">
                        <thead>
                            <tr>
                                <th scope="col" className="py-3.5 text-left text-sm font-semibold">{t("content.post")}</th>
                            ...
                            {reports.map(r => (
                                <tr key={r.id}>
                                    <td className="whitespace-nowrap py-4 text-sm font-semibold">
                                        <a target="_blank" href={route(`${r.reportable.type.toLowerCase()}s.show`, { post: r.reportable.slug })}>
                                            {r.reportable.title}
                                        </a>
                                    </td>
                                    ...
                                    <td className="relative whitespace-nowrap py-4 text-right text-sm font-medium sm:pr-0">
                                        <Menu as="div">
                                            ...
                                            >
                                                <MenuItem>
                                                    <Link
                                                        href={route("reports.posts.messages", { reportable: r.reportable.slug })}
                                                ...
                                                <MenuItem>
                                                    <button
                                                        onClick={() => setAction(route("posts.clearReports", { reportable: r.reportable.slug }))}
                                                ...
                                                <MenuItem>
                                                    <button
                                                        onClick={() => setAction(route("posts.destroy", { post: r.reportable.slug }))}
                                                        className="group flex w-full items-center gap-2 rounded-lg py-1.5 px-3 hover:bg-background-light dark:hover:bg-background-dark text-error-light dark:text-error-dark">
                                                        <Trash size={16} />
                                                        {t("content.delete")}
                                                    </button>
            ...
            }
            {
                ...
                <InfiniteScrollLoader
                    url={next_page_url}
                    onSuccess={
                        p => { setReports([...reports, ...(p.reports.items as GrouppedPostReport[])]) }
                    }
